...
namespace ILRuntime.Runtime.CLRBinding
{
    ...
    {
        internal static string GenerateFieldRegisterCode(this Type type, FieldInfo[] fields,
            HashSet<FieldInfo> excludes)
        {
            ...
            foreach (var i in fields)
            {
                ...
                sb.AppendLine(string.Format("            app.RegisterCLRFieldGetter(field, get_{0}_{1});", i.Name,
                    idx));
                if (!i.IsInitOnly && !i.IsLiteral)
                {
                    sb.AppendLine(string.Format("            app.RegisterCLRFieldSetter(field, set_{0}_{1});", i.Name,
                        idx));
                }
        ...
        internal static string GenerateFieldWraperCode(this Type type, FieldInfo[] fields, string typeClsName,
            HashSet<FieldInfo> excludes)
        {
            StringBuilder sb = new StringBuilder();
            ...
            foreach (var i in fields)
            {
                ...
                if (!i.IsInitOnly && !i.IsLiteral)
                {
                    sb.AppendLine(string.Format("        static void set_{0}_{1}(ref object o, object v)", i.Name,
                        idx));
                    sb.AppendLine("        {");
                    ...
                    else
                    {
                        if (CheckCanPinn(type))
                        {
                            ...
                            sb.AppendLine(string.Format("            {0}* p = ({0} *)(void *)h.AddrOfPinnedObject();",
                                typeClsName));
                        ...
                        {
                            sb.AppendLine(string.Format("            (({0})o).{1} = ({2})v;", typeClsName, i.Name,
                                realClsName));
                        }
                    }
