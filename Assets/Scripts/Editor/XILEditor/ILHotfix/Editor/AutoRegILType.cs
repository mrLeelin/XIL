...
//#endif
    static partial class AutoRegILType
    {
        ...
        static bool IsHotType(System.Type type)
        {
            ...
            if (type.Assembly.ManifestModule.Name == PathHelper.GetHotDll+".dll")
            {
            ...
            if (ns.Contains(PathHelper.GetHotNamespace))
            {
                HotTypes.Add(type, true);
                return true;
            }
...
#if USE_HOT
            string dll = string.Format(ResourcesPath.dataPath + "/../{0}",PathHelper.GetHotDllPath);
        ...
        static void BuildDelegate(Dictionary<System.Type, UseTypeInfo> csharpDelegate, Dictionary<System.Type, UseTypeInfo> hotTDic)
        {
            ...
            string file_path =  string.Format(PathHelper.GetAutoBuildDelegatePath, file_suffix);
